...
The local startup page shows the Matreshka nesting-doll mark, startup status, and available recovery actions; the loaded dsh renderer receives only the desktop protocol marker. Desktop does not open a plugin-manager window; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.
...
The loading page does not depend on the Host. Errors offer restart and reinstallation guidance. Disabling plugins and resetting Desktop are offered only when packaged application resources support profile recovery; development and early initialization failures expose restart alone. Windows and Linux register no application menu; macOS keeps only the system app menu. Runtime identity is checked before any backend starts; plugin changes have no automatic rollback.
