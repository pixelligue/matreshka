...
describe('official browser-brand plugin', () => {
  ...
  it('fills brand slots outside the official build profile', async () => {
  ...
  it('fills the conversation hero mark with the Matreshka logo', async () => {
    const subject = await bench()
    await subject.ctx.plugin({ inject: [...inject], apply }).await()
    expect(subject.slots.entries(HERO_HOLE)).toHaveLength(1)
    expect(subject.slots.entries(HERO_HOLE)[0]!.component).toBe(OfficialBrandMark)
  })

  it('renders the Matreshka name independently from both requested mark sizes', () => {
    const name = render(<OfficialBrandName />)
    expect(name.getByText('Matreshka')).toBeTruthy()
    name.unmount()

    const mark = render(<OfficialBrandMark size={34} />)
    const logo = mark.container.querySelector('[data-matreshka-logo-slot]')
    expect(logo?.tagName).toBe('IMG')
    expect(logo?.getAttribute('src')).toBe('/matreshka-logo.png')
    expect(logo?.getAttribute('width')).toBe('34')
    mark.rerender(<OfficialBrandMark size={24} />)
    expect(mark.container.querySelector('[data-matreshka-logo-slot]')?.getAttribute('width')).toBe('24')
  })
})
