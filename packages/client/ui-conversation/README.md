...
Under the card, the composer shows that Matrena reads UTF-8 text and source files, and that mp3, wav, ogg, m4a, aac, and webm can be played in the message. Sending playable audio keeps a local object URL so the sent message can play before the Host read.
