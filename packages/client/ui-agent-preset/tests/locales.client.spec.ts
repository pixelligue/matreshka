...
import { en, ru, presetDisplayText, zh } from '../src/client/locales.ts'
...
describe('preset display copy', () => {
  it.each([
  ...
  ] as const)('localizes the shipped %s preset in English, Chinese, and Russian', (id, nameKey, descriptionKey) => {
    ...
    expect(presetDisplayText(preset, translate(ru)))
      .toEqual({ name: ru[nameKey], description: ru[descriptionKey] })
    expect(ru[nameKey]).not.toBe(en[nameKey])
  })
