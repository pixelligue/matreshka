...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-plugins-matreshka PluginsCatalog',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-models SignOutRow id \'matreshka-sign-out\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-models MatreshkaSignInDialog id \'matreshka-sign-in\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-models ProfileFooter id \'matreshka-profile\'',
  ...
  {
    key: 'sidebar.footer.end',
    kind: 'list',
    scope: 'root',
    summary: 'Actions on the Settings row (end of the foot).',
    doc: 'Actions on the Settings row (end of the foot). Same owner share as\n`sidebar.footer.action`.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.footer.end\', () => ctx.slots.register(\n      { name: \'sidebar.footer.end\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-plugins-matreshka PluginsIcon',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: ask_user_question, bash, cordis_define, cordis_run, cordis_stop, cordis_undefine, edit, edit_image, generate_image, glob, grep, present, read, read_image, skill, todo_write, web_fetch, web_search, write',
    hookContext: '',
    slotInject: '',
    ...
    occupants: [
      ...
      'client-ui-tool GenerateImageRow key \'generate_image\'',
      'client-ui-tool GenerateImageRow key \'edit_image\'',
