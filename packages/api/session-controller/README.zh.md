...
`audio` 读取目标 Session 日志引用的一个可播放文件。接受的文件名以 mp3、wav、ogg、oga、m4a、aac 或 webm 结尾，且文件不得超过 32 MiB。响应包含持久文件引用、音频媒体类型和规范 base64。未引用的 id、其他文件名或更大的文件以 `session/attachment-invalid` 失败，且不会读取字节。模型仍然只收到保存路径。
