...
`audio` reads one playable file referenced by the addressed Session log. Accepted names end in mp3, wav, ogg, oga, m4a, aac, or webm, and the file must be at most 32 MiB. The response is the durable file reference, an audio media type, and canonical base64. An unreferenced id, another filename, or a larger file fails as `session/attachment-invalid` without a byte read. The model still receives only the saved path.
