...
describe('workspace context instruction discovery', () => {
  ...
  it('defaults dshHome and uses cwd itself as root when no project marker exists', async () => {
    ...
    // the home dirs point at an empty dir so the default ~/.matreshka holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.matreshka/AGENTS.md would otherwise leak in.
    vi.stubEnv('DSH_HOME', '')
  ...
  it('labels the default DSH home as ~/.matreshka when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.matreshka/AGENTS.md'), 'global default rule')
      ...
      expect(files.map(file => file.displayPath)).toEqual(['~/.matreshka/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as DSH_HOME instead of pretending it is ~/.matreshka', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
