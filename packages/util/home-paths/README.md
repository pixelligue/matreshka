---
description: "Shared resolution of the Matreshka home and user-data paths for packages that need one consistent root, tilde expansion, and stable watch paths."
...
`@deepseek-ai/dsh-home-paths` lets package authors resolve one Matreshka data root and derive child paths from it. An explicit path wins over `$DSH_HOME`, which wins over `~/.matreshka`; blank environment values are ignored. Its public helpers can render the root without revealing an absolute machine path, expand only bare or current-user tilde forms, and canonicalize watch targets whose final components do not yet exist. Use it as a direct library dependency, not through `cordis.yml`.
...
An explicit configured path has the highest precedence, then `$DSH_HOME`, then the default `~/.matreshka`. An empty or whitespace-only `$DSH_HOME` is treated as unset, so a blank override never resolves the home to the current working directory.
...
For user-facing paths, render the root symbolically rather than as a machine path: the default home displays as `~/.matreshka` and any configured home displays as `$DSH_HOME`. The display form never leaks an absolute machine path.
...
`resolveDshHome` reads the explicit override, then `$DSH_HOME`, then falls back to the operating-system home joined with `.matreshka`. The chosen value is tilde-expanded and normalized to an absolute path; `dshHomePath` joins child segments with Node's platform path rules. `dshHomeDisplay` compares the resolved path against the default root and returns the symbolic label, so a configured home never leaks its absolute path.
