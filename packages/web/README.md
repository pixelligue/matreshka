...
Ten packages play the web roles; the subsystem reference owns the exhaustive vocabulary and contracts.
...
| [`images-matreshka/`](images-matreshka/README.md) | Posts image generate/edit to the Matreshka API with the session token | registers on `ctx.tools` |
| [`audio-matreshka/`](audio-matreshka/README.md) | Transcribes a saved recording with Deepgram Nova-3 through the Matreshka API | registers on `ctx.tools` |
