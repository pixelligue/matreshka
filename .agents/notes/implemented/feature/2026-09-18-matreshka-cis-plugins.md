...
- **Tab.** `@deepseek-ai/dsh-client-ui-plugins-matreshka` registers `sidebar.panellist` id `plugins` at order 10 (under New session, above Workspaces) and a keyed `main` occupant `plugins`. Selecting it hides chat; New session and workspace session selection already call `layout.selectPanel(null)` and restore chat. Copy is locale-owned (RU/EN/ZH). The pane also has Skills and MCP tabs, recorded in [Plugins page skills and MCP tabs](2026-09-22-plugins-skills-mcp-tabs.md).
