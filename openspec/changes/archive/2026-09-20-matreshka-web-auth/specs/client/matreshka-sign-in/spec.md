...
Packaged Desktop MUST NOT collect email or password. It MUST offer a control that opens the public site login with a desktop handoff. Unpackaged Desktop and the web GUI without the Desktop auth bridge MUST keep email and password fields that post `POST {apiOrigin}/v1/auth/login`.
...
- **WHEN** unpackaged Desktop or the web GUI submits a valid email and password
...
- **WHEN** packaged Desktop has no session and the operator activates the website sign-in control
...
- **WHEN** packaged Desktop is waiting for sign-in and the shell delivers a one-time code
