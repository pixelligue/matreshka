...
The API MUST accept only the public id `matrena`. It MUST map that id to the upstream model internally. Vendor ids MUST return 400 without calling the upstream. SSE chunks returned to the client MUST use `model` `matrena`, never the upstream vendor id.
